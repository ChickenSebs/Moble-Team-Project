...
namespace calendar4
{
    public partial class mainForm : Form
    {
        ...
        private void InitThemeMenu()
        {
            ...
            themeMenu.Items.Add("Light", null, (s, e) => ApplySelectedTheme(AppTheme.Light));
            themeMenu.Items.Add("Dark", null, (s, e) => ApplySelectedTheme(AppTheme.Dark));

            themeMenu.Items.Add(new ToolStripSeparator());

            themeMenu.Items.Add("🌸 Blossom Premium", null, (s, e) => ApplySelectedTheme(AppTheme.Blossom));
            themeMenu.Items.Add("🍀 Mint Premium", null, (s, e) => ApplySelectedTheme(AppTheme.Mint));
            themeMenu.Items.Add("☁ Lavender Premium", null, (s, e) => ApplySelectedTheme(AppTheme.Lavender));
            themeMenu.Items.Add("🧸 Cozy Premium", null, (s, e) => ApplySelectedTheme(AppTheme.Cozy));
        }
        ...
        private void InitCalendarViewOptions()
        {
            if (tool_month != null)
                tool_month.Click += (_, _) =>
                    ChangeCalendarView(CalendarControl.CalendarViewMode.Month);

            if (tool_week != null)
                tool_week.Click += (_, _) =>
                    ChangeCalendarView(CalendarControl.CalendarViewMode.Week);

            if (tool_day != null)
                tool_day.Click += (_, _) =>
                    ChangeCalendarView(CalendarControl.CalendarViewMode.Day);

            UpdateCalendarViewMenu();
        }

        private void ChangeCalendarView(CalendarControl.CalendarViewMode viewMode)
        {
            foreach (TabPage tab in tabControl1.TabPages)
            {
        ...
        {
            CalendarControl.CalendarViewMode currentMode = GetSelectedViewMode();

            if (tool_month != null)
                tool_month.Checked = currentMode == CalendarControl.CalendarViewMode.Month;

            if (tool_week != null)
                tool_week.Checked = currentMode == CalendarControl.CalendarViewMode.Week;

            if (tool_day != null)
                tool_day.Checked = currentMode == CalendarControl.CalendarViewMode.Day;
        }

        private CalendarControl.CalendarViewMode GetSelectedViewMode()
        {
            if (tabControl1.SelectedTab?.Controls.Count > 0)
            {
                Control selectedControl = tabControl1.SelectedTab.Controls[0];
        ...
        {
            if (this.Controls.Find("monthCalendar1", true).Length > 0)
            {
                MonthCalendar miniCal =
                    this.Controls.Find("monthCalendar1", true)[0] as MonthCalendar;

                if (miniCal != null)
                    miniCal.DateChanged += MiniCal_DateChanged;
            }
        }
        ...
        private void RefreshAllViews()
        {
            foreach (TabPage tab in tabControl1.TabPages)
            {
                ...
                {
                    diaryCtrl.SetTargetDate(currentMonth);
                }
        ...
        private void UpdateSummaryView()
        {
            ...
            var summaryBoxes = Controls.Find("richTextBox1", true);
            if (summaryBoxes.Length == 0 || summaryBoxes[0] is not RichTextBox summaryBox)
                return;

            if (tabControl1.SelectedTab.Controls.Count > 0 &&
                tabControl1.SelectedTab.Controls[0] is DiaryControl diaryControl)
            {
                summaryBox.Text = summaryService.CreateDiarySummary(diaryControl.GetDiaryMap());
                return;
            }
        ...
        private void UpdateCalendarTitle()
        {
            ...
            lbmain_title.Text = CalendarTitleFormatter.Format(
                currentMonth,
                GetSelectedViewMode());
        }
        ...
        private void AddNewCustomTab(string title, TabType type)
        {
            ...
            TabPage newTab = CreateTabPage(title, type);
        ...
        private void tabControl1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                for (int i = 0; i < tabControl1.TabPages.Count; i++)
                {
                    ...
                    {
                        TabPage clickedTab = tabControl1.TabPages[i];
        ...
        private TabPage CreateTabPage(string title, TabType type)
        {
            ...
            UiThemeService.ApplyTheme(newTab);
        ...
        private void SaveTabs()
        {
            var tabs = tabControl1.TabPages
                .Cast<TabPage>()
                .Where(tab => tab.Text != "+")
                .Select(tab => new TabData
                {
                    Title = tab.Text,
                    Type = tab.Tag is TabType type ? type : TabType.Calendar
                })
                .ToList();

            tabRepository.Save(tabs);
        }

        private void LoadTabs()
        {
            var tabs = tabRepository.Load();
            if (tabs.Count == 0)
            {
            ...
            else
            {
                ...
                foreach (var tab in tabs)
                    CreateTabPage(tab.Title, tab.Type);
            }
        ...
        private void tool_theme_Click(object sender, EventArgs e)
        {
            if (themeMenu == null)
                return;
